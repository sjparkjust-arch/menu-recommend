...
- **MY밥픽 찜한 메뉴 5개로 축소 + 랜덤픽 버튼 제거**
  대시보드 MY밥픽의 찜한 메뉴 표시를 최근 좋아요 순 10개 → **5개**로 줄임(`MY_BOBPICK_LIMIT`). "🎲 이 중에 랜덤으로 골라줌" 버튼과 관련 JS·결과 표시 영역·JSON 임베드를 전부 제거(미니 통계 줄·선호 배지·찜한 메뉴 리스트·"전체보기" 링크는 유지).

- **음식분류(요리종류) 9종 → 6종 재편**
  한식/중식/일식/양식/아시안/패스트푸드로 통일. 없어진 3종(멕시칸·인도·치킨, 총 5개 메뉴)을 재배치: 타코·퀘사디아(멕시칸)→양식, 버터치킨커리(인도)→아시안, 양념치킨·후라이드 치킨(치킨)→패스트푸드. 데이터 마이그레이션 `menus.0002_remap_cuisines`(RunPython)로 5개 메뉴 재배정 후 남은 3종 Cuisine 삭제(`Menu.cuisine`은 PROTECT라 재배치 후에만 삭제 가능, `UserPreference.cuisine`은 CASCADE라 관련 선호도 39건이 함께 자동 정리됨). `seed_data.py`의 `MENUS` 소스도 함께 수정해 새 환경에서 처음부터 6종으로 생성되도록. 국가별 필터 체크박스 6개로 축소, 전체 메뉴 199개·분포는 한식88/양식44/일식30/중식23/패스트푸드9/아시안5.
...
- **게임 3종 신규: 메뉴 돌림판 · 메뉴 사다리타기 · 음식 이상형 월드컵**
  ...
  - **🎯 메뉴 돌림판**: 메인 코스에서 8개 랜덤 추출 → Canvas 2D로 8등분 원판(브랜드 주황/잉크 교대색)을 그리고, "돌리기" 클릭 시 CSS `transform: rotate()` 애니메이션 → 정지 각도로 당첨 조각을 판정해 결과 텍스트+상세링크 표시. 각도→인덱스 판정 로직은 0~359도 전 구간 검증(8개 슬라이스가 정확히 45개씩 균등 분포 확인).
  - **🪜 메뉴 사다리타기**: 인원수(4/6/8) 선택 → Canvas 2D로 세로선+랜덤 가로선(사다리) 그리고, 참가자 이름 입력 후 시작 버튼 클릭 시 사다리 규칙에 따라 경로를 계산해 애니메이션으로 이동, 도착 메뉴를 결과로 표시. 사다리 생성 로직(같은 행에서 인접 가로줄이 세로줄을 겹쳐 쓰지 않게)은 4/6/8명 각 2000회 시뮬레이션으로 **항상 유효한 1:1 매핑(순열)**임을 검증.
  - **🏆 음식 이상형 월드컵**: 강수(8/16/32강) 선택 → 카드 대결 토너먼트(순수 DOM/JS, 이미지 없으면 기존 placeholder 패턴), 라운드가 줄어들며 최종 우승 메뉴를 결과로 표시.
